...
xmlXPathNewCompExpr(void) {
    ...
    cur->maxStep = 10;
    cur->nbStep = 0;
    cur->steps = (xmlXPathOp *) xmlMalloc(cur->maxStep *
	                                   sizeof(xmlXPathOp));
    if (cur->steps == NULL) {
	xmlFree(cur);
	return(NULL);
    }
    memset(cur->steps, 0, cur->maxStep * sizeof(xmlXPathOp));
    cur->last = -1;
    return(cur);
}
 ...
 */
static void
xmlXPathCompOpSetEvalMode(xmlXPathParserContextPtr ctxt, int opIndex,
                          xmlXPathEvalMode mode, int isPred) {
    xmlXPathOpPtr op = &ctxt->comp->steps[opIndex];
    int oldMode;
 ...
 */
static xmlXPathOp *
xmlXPathCompAdd(xmlXPathParserContextPtr ctxt, xmlXPathOpcode opcode,
                xmlXPathObjectType retType) {
    xmlXPathCompExprPtr comp = ctxt->comp;
    xmlXPathOpPtr op;
...
xmlXPathCompAddUnary(xmlXPathParserContextPtr ctxt, xmlXPathOpcode opcode,
                     xmlXPathObjectType retType, int ch1) {
    xmlXPathOpPtr op;

    op = xmlXPathCompAdd(ctxt, opcode, retType);
    if (op == NULL)
        return(NULL);
...
static xmlXPathOpPtr
xmlXPathCompAddBinary(xmlXPathParserContextPtr ctxt, xmlXPathOpcode opcode,
 ...
 */
static xmlXPathObjectPtr
xmlXPathCacheNewNodeSet(xmlXPathParserContextPtr pctxt)
{
    xmlXPathContextCachePtr cache = pctxt->context->cache;
    xmlXPathObjectPtr ret;
    xmlNodePtr *nodes;
    ...
    ret->type = XPATH_NODESET;
    ret->boolval = 0;
    ret->nodesetval->nodeTab = nodes;
    ret->nodesetval->nodeNr = 0;
    ret->nodesetval->nodeMax = XML_NODESET_DEFAULT;
...
xmlXPathCacheObjectCopy(xmlXPathParserContextPtr pctxt, xmlXPathObjectPtr val)
{
	...
	switch (val->type) {
            case XPATH_NODESET: {
                ...
                ret = xmlXPathCacheNewNodeSet(pctxt);
                if (ret == NULL)
                    return(NULL);
...
static int
xmlXPathGrowValueTable(xmlXPathParserContextPtr ctxt) {
...
static void
xmlXPathValuePushInternal(xmlXPathParserContextPtr ctxt,
...
static xmlXPathObjectPtr
xmlXPathValuePopInternal(xmlXPathParserContextPtr ctxt) {
    ctxt->valueNr--;
    ...
    return(ctxt->valueTab[ctxt->valueNr]);
}
...
static void
xmlXPathValueSet(xmlXPathParserContextPtr ctxt, xmlXPathObjectPtr value) {
    ctxt->valueTab[ctxt->valueNr-1] = value;
    ctxt->value = value;
}
...
static void
xmlXPathNodeSetKeep(xmlNodeSetPtr set, int index) {
    int i;
    ...
    if (index < set->nodeNr) {
        set->nodeTab[0] = set->nodeTab[index];
        set->nodeNr = 1;
    } else {
        set->nodeNr = 0;
    }
 ...
 */
static void
xmlXPathNodeSetFindFirst(xmlNodeSetPtr set, xmlXPathEvalMode mode) {
    xmlNodePtr node;
    int wanted, index, i;
    ...
    index = 0;
    node = set->nodeTab[0];
    for (i = 1; i < set->nodeNr; i++) {
        xmlNodePtr other = set->nodeTab[i];
        int res;
 ...
 */
static void
xmlXPathNodeSetFinish(xmlNodeSetPtr set, xmlXPathEvalMode mode) {
    xmlNodePtr prev;
    int i, j;
    int pos = 0;
...
ATTRIBUTE_NO_INLINE
static int
xmlXPathNodeSetMergeAndClear(xmlXPathParserContextPtr ctxt,
                             xmlNodeSetPtr set1, xmlNodeSetPtr set2,
                             xmlXPathEvalMode mode) {
	...
	for (i = 0;i < set2->nodeNr;i++) {
	    ...
	    */
            if (set1->nodeNr >= set1->nodeMax) {
                if (xmlXPathCacheNodeSetGrow(ctxt->context, set1) < 0) {
                    xmlXPathNodeSetClear(set2, 1);
                    return(-1);
                }
            }
...
xmlXPathBooleanFuncInternal(xmlXPathParserContextPtr ctxt) {
    ...
    obj = ctxt->value;
    boolval = xmlXPathCastToBoolean(obj);
    xmlXPathReleaseObject(ctxt->context, obj);
    xmlXPathValueSet(ctxt, xmlXPathCacheNewBoolean(ctxt, boolval));
}
...
xmlXPathNumberFuncInternal(xmlXPathParserContextPtr ctxt) {
    ...
    obj = ctxt->value;
    floatval = xmlXPathCastToNumberInternal(ctxt, obj);
    xmlXPathReleaseObject(ctxt->context, obj);
    xmlXPathValueSet(ctxt, xmlXPathCacheNewFloat(ctxt, floatval));
}
...
xmlXPathRoot(xmlXPathParserContextPtr ctxt) {
    ...
    result = xmlXPathCacheNewNodeSet(ctxt);
    if (result == NULL)
        return;

    doc = xmlXPathGetRoot(ctxt->context);
    if (xmlXPathCacheNodeSetAdd(ctxt->context, result->nodesetval,
                                (xmlNodePtr) doc) < 0) {
        xmlXPathPErrMemory(ctxt);
        return;
    }
...
xmlXPathIdFunction(xmlXPathParserContextPtr ctxt, int nargs) {
    ...
    ret = xmlXPathCacheNewNodeSet(ctxt);
    if (ret == NULL) {
        xmlXPathPErrMemory(ctxt);
        goto error;
    }
	...
	int i;
        ...
        for (i = 0; i < obj->nodesetval->nodeNr; i++) {
            ...
            if (xmlXPathAddElementsByIds(ctxt->context, set, doc,
                                         tokens) < 0) {
                xmlXPathPErrMemory(ctxt);
                goto error;
            }
        ...
        if (xmlXPathAddElementsByIds(ctxt->context, set, doc, tokens) < 0) {
            xmlXPathPErrMemory(ctxt);
            goto error;
        }
    }
...
xmlXPathCompFunctionCall(xmlXPathParserContextPtr ctxt) {
    ...
    if ((sfunc != NULL) && (sfunc->compiler != NULL)) {
        ...
        if (nbargs == 0)
            ...
            op = xmlXPathCompAddUnary(ctxt, opcode, type, ctxt->comp->last);
        if (op == NULL)
            goto error;
 ...
 *
 * Returns 1 if the predicate was compiled, 0 if it was fused into
 * another operation.
 */
static int
xmlXPathCompPredicate(xmlXPathParserContextPtr ctxt, int filter) {
    ...
    SKIP_BLANKS;
    if (CUR != '[') {
	XP_ERROR0(XPATH_INVALID_PREDICATE_ERROR);
    }
    NEXT;
    SKIP_BLANKS;
    ...
    if (mode != XPATH_EVAL_ALL) {
        ...
        xmlXPathCompAddBinary(ctxt, opcode, XPATH_NODESET,
                              ch1, ctxt->comp->last);
...
xmlXPathCompStep(xmlXPathParserContextPtr ctxt) {
	...
	SKIP_BLANKS;
        predIndex = -1;
	while (CUR == '[') {
	    if (xmlXPathCompPredicate(ctxt, 0) > 0) {
...
static void
xmlXPathCompOpEvalNodeset(xmlNodeSetPtr set, const xmlXPathOp *op,
                          xmlXPathEvalMode mode) {
    ...
    switch (opMode) {
        ...
        case XPATH_EVAL_FIRST:
            if (set->nodeNr > 1)
                xmlXPathNodeSetKeep(set, 0);
            break;

        case XPATH_EVAL_LAST:
            if (set->nodeNr > 1)
                xmlXPathNodeSetKeep(set, set->nodeNr - 1);
            break;

        default:
            if (set->nodeNr > 0)
                xmlXPathNodeSetKeep(set, opMode - 1);
            break;
    }
}
	...
	hasNsNodes = 0;
        ...
        while (cur != NULL) {
            ...
            if (outSeq->nodeNr >= outSeq->nodeMax) {
                if (xmlXPathCacheNodeSetGrow(ctxt->context, outSeq) < 0) {
                    xmlXPathPErrMemory(ctxt);
                    break;
                }
            }
...
empty_set:
     ...
     */
    while (inputIdx < inputSize) {
        xmlNodePtr cur = inputNodes[inputIdx++];
...
ATTRIBUTE_NO_INLINE
static xmlXPathObjectPtr
xmlXPathCompOpEvalFunc(xmlXPathParserContextPtr ctxt, const xmlXPathOp *op) {
    xmlXPathContextPtr xpctxt = ctxt->context;
    xmlXPathFunction func = NULL;
    ...
    return(xmlXPathValuePopInternal(ctxt));
}

ATTRIBUTE_NO_INLINE
static void
xmlXPathCompOpEvalSort(xmlNodeSetPtr set, const xmlXPathOp *op,
                       xmlXPathEvalMode mode) {
    xmlXPathEvalMode opMode;
    ...
    switch (opMode) {
        ...
        case XPATH_EVAL_ALL:
            if (set->nodeNr > 1) {
                ...
                xmlXPathNodeSetFinish(set, opMode);
            }
            break;
        ...
        case XPATH_EVAL_LAST:
            if (set->nodeNr > 1)
                xmlXPathNodeSetFindFirst(set, opMode);
            break;
 ...
 */
static xmlXPathObjectPtr
xmlXPathCompOpEval(xmlXPathParserContextPtr ctxt, int opIndex,
                   xmlXPathEvalMode mode) {
    const xmlXPathOp *op;
    xmlXPathContextPtr xpctxt = ctxt->context;
    xmlXPathObjectPtr result;
    ...
    switch (op->op) {
        case XPATH_OP_BOOL: {
            ...
            result = xmlXPathCompOpEval(ctxt, op->ch1, XPATH_EVAL_DEFAULT);
            if (result == NULL)
                break;

            boolval = xmlXPathCastToBoolean(result);
            xmlXPathReleaseObject(xpctxt, result);
            result = xmlXPathCacheNewBoolean(ctxt, boolval);
            break;
        }
        ...
        case XPATH_OP_NUMBER: {
            ...
            floatval = xmlXPathCastToNumberInternal(ctxt, result);
            xmlXPathReleaseObject(xpctxt, result);
            result = xmlXPathCacheNewFloat(ctxt, floatval);
            break;
        }
        ...
        case XPATH_OP_STRING: {
            ...
            stringval = xmlXPathCastToString(result);
            xmlXPathReleaseObject(xpctxt, result);
            if (stringval == NULL) {
                xmlXPathPErrMemory(ctxt);
                break;
            ...
            result = xmlXPathCacheWrapString(ctxt, stringval);
            break;
        }
        ...
        case XPATH_OP_NODESET:
            ...
            xmlXPathCompOpEvalNodeset(result->nodesetval, op, mode);
            break;
        ...
        case XPATH_OP_OR: {
            ...
            if (result->boolval == breakVal)
                break;
            xmlXPathReleaseObject(xpctxt, result);

            result = xmlXPathCompOpEval(ctxt, op->ch2, XPATH_EVAL_DEFAULT);
            break;
        }
        ...
        case XPATH_OP_NE: {
            ...
            arg = xmlXPathCompOpEval(ctxt, op->ch1, XPATH_EVAL_DEFAULT);
            if (arg == NULL)
                break;
            result = xmlXPathCompOpEval(ctxt, op->ch2, XPATH_EVAL_DEFAULT);
            if (result == NULL) {
                xmlXPathReleaseObject(xpctxt, arg);
                break;
            }
        ...
        case XPATH_OP_GE: {
            ...
            ret = xmlXPathCompareValuesInternal(ctxt, arg, result, inf, strict);
            xmlXPathReleaseObject(xpctxt, arg);
            xmlXPathReleaseObject(xpctxt, result);
            result = xmlXPathCacheNewBoolean(ctxt, ret);
            break;
        ...
        case XPATH_OP_ROUND:
            ...
            switch (op->op) {
                case XPATH_OP_NEG:
                    ...
                    break;
                case XPATH_OP_FLOOR:
                    result->floatval = floor(result->floatval);
                    break;
                case XPATH_OP_CEIL:
                    result->floatval = ceil(result->floatval);
                    break;
                case XPATH_OP_ROUND:
                    result->floatval = xmlXPathRound(result->floatval);
                    break;
                default:
                    break;
            }
        ...
        case XPATH_OP_MOD: {
            ...
            arg = xmlXPathCompOpEval(ctxt, op->ch2, XPATH_EVAL_DEFAULT);
            if (arg == NULL)
                break;
            val2 = arg->floatval;
            xmlXPathReleaseObject(xpctxt, arg);
            ...
            switch (op->op) {
                ...
                case XPATH_OP_DIV:
                    ...
                    break;
                case XPATH_OP_MOD:
                    result->floatval = fmod(result->floatval, val2);
                    break;
                default:
                    break;
            }
        ...
        case XPATH_OP_UNION: {
            ...
            arg = xmlXPathCompOpEval(ctxt, op->ch2, mode);
            if (arg == NULL)
                break;

            if (((mode == XPATH_EVAL_ANY) || (op->mode == XPATH_EVAL_ANY)) &&
                 (arg->nodesetval->nodeNr > 0)) {
                result = arg;
                break;
            }

            result = xmlXPathCompOpEval(ctxt, op->ch1, mode);
            if (result == NULL) {
                xmlXPathReleaseObject(xpctxt, arg);
                break;
            }
	    ...
	    if ((mode != XPATH_EVAL_ANY) &&
                ...
                if (result->nodesetval->nodeNr <= 0) {
                    ...
                    if (xmlXPathNodeSetMergeAndClear(ctxt, result->nodesetval,
                                                     arg->nodesetval,
                                                     mode) < 0)
                        xmlXPathPErrMemory(ctxt);
                }
        ...
        case XPATH_OP_ROOT: {
            ...
            doc = xmlXPathGetRoot(ctxt->context);
            if (doc != NULL) {
                xmlNodeSetPtr set = result->nodesetval;

                set->nodeTab[0] = (xmlNodePtr) doc;
                set->nodeNr = 1;
            }
            break;
        }
        ...
        case XPATH_OP_STEP:
            ...
            result = xmlXPathCompOpEvalStep(ctxt, result, op, mode);
            break;

        case XPATH_OP_VALUE:
            result = xmlXPathCacheObjectCopy(ctxt, op->as.obj);
            break;
        ...
        case XPATH_OP_VARIABLE: {
            ...
            if (xpctxt->varLookupFunc != NULL)
                result = xpctxt->varLookupFunc(xpctxt->varLookupData,
                                               op->qname.name, URI);

            if ((result == NULL) && (xpctxt->varHash != NULL)) {
                result = xmlHashLookup2(xpctxt->varHash, op->qname.name, URI);
                if (result != NULL) {
                    result = xmlXPathCacheObjectCopy(ctxt, result);
                    if (result == NULL) {
                        xmlXPathPErrMemory(ctxt);
                        break;
                    }
        ...
        case XPATH_OP_SFUNC: {
            ...
            result = xmlXPathValuePopInternal(ctxt);
            break;

sfunc_cleanup:
            while (ctxt->valueNr > frame)
                xmlXPathReleaseObject(xpctxt, xmlXPathValuePopInternal(ctxt));
            break;
        }
        ...
        case XPATH_OP_FUNCTION: {
            ...
            result = xmlXPathCompOpEvalFunc(ctxt, op);
...
func_cleanup:
            while (ctxt->valueNr > frame)
                xmlXPathFreeObject(xmlXPathValuePopInternal(ctxt));
            break;
        }
        ...
        case XPATH_OP_FILTER:
            ...
            xmlXPathNodeSetFilter(ctxt, op, result->nodesetval, 0, mode, 1);
            break;
        ...
        case XPATH_OP_SORT:
            ...
            if (result->type == XPATH_NODESET)
                xmlXPathCompOpEvalSort(result->nodesetval, op, mode);
            break;
        ...
        case XPATH_OP_LAST: {
            ...
            if (contextSize < 0) {
                xmlXPathErr(ctxt, XPATH_INVALID_CTXT_SIZE);
                break;
            }

            result = xmlXPathCacheNewFloat(ctxt, contextSize);
            break;
        }
        ...
        case XPATH_OP_POSITION: {
            ...
            result = xmlXPathCacheNewFloat(ctxt, position);
            break;
        }
			    ...
			    int isPredicate)
{
    ...
    resObj = xmlXPathCompOpEval(ctxt, opIndex, XPATH_EVAL_ANY);
    if (resObj == NULL)
        return(-1);
 ...
 */
static int
xmlXPathRunEval(xmlXPathParserContextPtr ctxt, xmlXPathObjectPtr *resObjPtr,
                int toBool)
{
...
#ifdef XPATH_STREAMING
    if (ctxt->comp->stream)
	return(xmlXPathRunStreamEval(ctxt, ctxt->comp->stream, resObjPtr,
                                     toBool));
#endif
	...
	resObj = xmlXPathCompOpEval(ctxt, comp->last, XPATH_EVAL_DEFAULT);
        if (resObj != NULL) {
            *resObjPtr = resObj;
            res = 0;
        } else {
			     ...
			     int toBool)
{
    ...
    res = xmlXPathRunEval(pctxt, resObjPtr, toBool);
